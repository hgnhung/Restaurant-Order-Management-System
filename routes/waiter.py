        ...
        if order_id:
            for item in cart_items:
                order_model.add_order_details(
                    ...
                    special_note=str(item['note'])
                )
            temporary_cart[str(table_number)] = []
            return redirect('/waiter/tables')
        else:
            return "Error during order creation!", 500
    except Exception as e:
        print("--- CONFIRM ORDER ROUTE ERROR ---:", e)
        return "System processing error!", 500
    finally:
        conn.close()


# 6. ĐÃ THÊM LẠI: Chức năng xem chi tiết đơn hàng (Giao diện: order_detail.html)
...
def order_detail():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT orderID FROM Orders WHERE tableNumber = ? AND status = 'Confirmed'", (int(table_number),))
        row = cursor.fetchone()
        ...
        return render_template('waiter/order_detail.html', table_number=table_number, items=items)
    except Exception as e:
